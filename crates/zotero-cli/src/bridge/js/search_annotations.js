...
var filtered = (P.colors && P.colors.length) ? annots.filter(a => P.colors.includes(a.annotationColor)) : annots;
return filtered.slice(0, P.limit).map(a => {
  ...
  return {type: a.annotationType, text: (a.annotationText || '').substring(0, 200),
    comment: a.annotationComment || '', color: a.annotationColor || '',
    page: a.annotationPageLabel || '', parentTitle: title};
});
