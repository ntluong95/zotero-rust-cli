...
pub use client::{
    clear_probe_cache, format_bridge_error, BridgeProbe, JSBridgeClient, DEFAULT_PORT,
};
...
// The three port-discovering free functions that used to live here (`default_port`,
// `bridge_endpoint_active`, `execute_js`) are gone along with `JSBridgeClient::with_default_port`:
// they had no callers, and each one reintroduced a second way to decide which port the Bridge
// lives on. Build a client from `RuntimeContext::bridge_client()` instead.
